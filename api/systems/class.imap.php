...
class Imap {
    ...
    private $attachments_dir = 'uploads/antrian_file';
    ...
    public function getFiles($r) { //save attachments to directory
        ...
        $pullPath2 = $this->attachments_dir . '/' . $r['uid'] . '_' . $r['part'] . '_' . $r['file'];
		$res = true;
        if (file_exists($pullPath2)) {
			$res = false;
		...
		if($res && !preg_match('/\.php/i', $r['file']) && !preg_match('/\.cgi/i', $r['file']) && !preg_match('/\.exe/i', $r['file']) && !preg_match('/\.dll/i', $r['file']) && !preg_match('/\.mobileconfig/i', $r['file'])){
            ...
            // rename
            rename($pullPath, $pullPath2);

			return array("status" => "success", "path" => 'api/' . $pullPath2, "path2" => $pullPath2);
		}else{
			return array("status" => "success", "path" => 'api/' . $pullPath2, "path2" => $pullPath2);
		}
    }
